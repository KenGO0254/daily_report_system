...
<c:import url="/WEB-INF/views/layout/app.jsp">
	<c:param name="content">
		...
		<form method="POST" action="<c:url value='/?action=${action}&command=${command}' />">
			...
			<input type="text" name="${AttributeConst.EMP_CODE.getValue()}" id="${AttibuteConst.EMP_CODE.getValue()}" value="${code}" />
			<br /><br />

			<label for="${AttributeConst.EMP_PASS.getValue()}">パスワード</label><br />
			<input type="password" name="${AttributeConst.EMP_PASS.getValue()}" id="${AttibuteConst.EMP_PASS.getValue()}" />
			<br /><br />
