...
/*
 * 従業員のテーブル操作に関わる処理を行うクラス
 */
public class EmployeeService extends ServiceBase {
	/**
	 * 指定されたページ数の一覧画面に表示するデータを取得し、EmployeeViewのリストを返却する
	 * @param page ページ数
	 * @return 表示するデータのリスト
	 */
	public List<EmployeeView> getPerPage(int page) {
		List<Employee> employees = em.createNamedQuery(JpaConst.Q_EMP_GET_ALL, Employee.class)
				.setFirstResult(JpaConst.ROW_REP_PAGE * (page - 1))
				.setMaxResults(JpaConst.ROW_REP_PAGE)
				.getResultList();
	...
	/*
	 * idを条件に取得したデータをEmployeeViewのインスタンスに返却する
	 * @param id
	 * @return 取得データのインスタンス
	 */
	...
	public List<String> update(EmployeeView ev, String pepper) {
		...
		savedEmp.setAdminFlag(ev.getAdminFlag()); //変更後の管理者クラスを設定する
